frappe.ui.form.on('Sales Order', {
    before_load: function(frm) {
        ...
        frm._deleting_row = false; // علامة لتتبع عملية الحذف
    },
    ...
    refresh: function(frm) {
        ...
        // إضافة حدث للحذف لمعالجة حذف الصفوف
    ...
    custom_discount_amount_emp: function(frm) {
        ...
        // توزيع الخصم على الصفوف
        distribute_discount(frm, total_discount);
    },
    
    // إضافة حدث للتحقق بعد إزالة الصف
...
function distribute_discount(frm, total_discount) {
    ...
    if (item_count === 0 && total_discount > 0) {
        // إذا لم توجد أصناف ولكن هناك خصم، إعادة تعيين الخصم إلى الصفر
...
function validate_total_discount(frm) {
    ...
    let total_row_discount = frm.doc.items.reduce((sum, row) => sum + (row.custom_discount || 0), 0);
    total_row_discount = Math.round(total_row_discount * 100) / 100; // تجنب أخطاء الفاصلة العائمة
